...
standimage.src = "img/mark.png";

var cactus = new Image();
cactus.src = "img/stolb1.png";

var cactus2image = new Image();
cactus2image.src = "img/stolb2.png";
...
function randGap() {
    return Math.floor(150 + Math.random() * (500 - 200 + 1));
}
var scoreText = {
    x: 220,
    y: 200,
...
var player = {
    ...
    update: function () {
        if (this.y < 470) {
            gamearea.context.drawImage(standimage, this.x, this.y - 50, 100, 100);
        }
        else {
            gamearea.context.drawImage(standimage, this.x, this.y - 50, 100, 100);
        }
...
function obstacle() {
    ...
    this.draw = function () {
        if (this.enemytype == "cactus") {
            gamearea.context.drawImage(cactus, this.x, this.y - 50, 100, this.height + 40);
        }
        else if (this.enemytype == "cactuss") {
            gamearea.context.drawImage(cactus2image, this.x, this.y - 50, 100, this.height + 40);
        }
        else {
...
var gamearea = {
    ...
    stop: function () {
        ...
        gamearea.context.fillText("В КУЗОВНОЙ ЦЕХ!", 200, 250);
    },
}